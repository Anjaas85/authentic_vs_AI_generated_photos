...
def evaluate_model(model, dataloader, device, phase="validation", metrics_file_path="metrics/metrics.txt"):
    """
    Evaluates the model on the given dataloader and writes metrics to a file.

    Args:
        model: Trained PyTorch model.
        dataloader: DataLoader for the dataset to evaluate.
        device: Device to use for computation ('cpu' or 'cuda').
        phase: Phase of evaluation ('validation' or 'test').
        metrics_file_path: Path to save the evaluation metrics.
    """
    model.eval()
    all_preds = []
    all_labels = []
